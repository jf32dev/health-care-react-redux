...
import { getECG, getSteps } from '../../redux/actions';
        ...
        for(let j=0; j<24; j++){
          ...
          if(m===0){
            ...
            for(let j=0; j<filter_hour.length; j++){
              hourly_value += filter_hour[j][rate_string[m]];
            }
            status_array_temp[m].push(Math.floor(hourly_value));
            total_step+=hourly_value;
          }
        }
        total_steps[m] = total_step;
      } else if(current_range_type === "weekly") {
    ...
    return (
      <Card>
        <CardBody>
          <div className="fixed_header">
            ...
            <div className="chart_total_value">
              <div className="show_total_step_div">
                <label className="total_steps_header">Step: </label>
                <p><label className="total_steps_value">{data[1].show_total}</label>
                <label className="chart_unit"> Steps</label></p>
              </div>
            </div>
          </div>
          ...
          <div className="chart_total ">
            <div className="chart-info v-c">
              <p className="step_date_range">Steps, {show_from}  ~ {show_to}</p>
              {/* <HeartModal type="Step Count"/> */}
            </div>
            <div className="chart_div v-c">
              <button onClick={e => this.moveDate('prev')} className={`step_date_button webview_prev prev_button_1`} > <label>&#x2039;</label></button>
              <div className="chart_desktop">
...
const mapStateToProps = (state) => {
  return {
    ...
    step:state.heartRate.steps,
  }
}
export default connect(mapStateToProps, { getECG, getSteps})(Step);