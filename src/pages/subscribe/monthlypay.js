...
class MonthlyPay extends Component {
	...
	checkPayment = () => {
		...
		return (Object.keys(info).length > 0 && info.type === 1) ? true : false;
	}
    ...
    if (payload.error) {
		...
		const response = await subscribeApi({payment_id : payload.paymentMethod.id, type:1});
		if (!response.data) {
			this.setState({
				processing: false,
...
export default withRouter(connect(mapStateToProps, { getActivePayment })(withSnackbar(MonthlyPay)));
